...
TYPED_TEST(GridFixture, MaxIndex)
{
    const auto& dim = this->m_Grid.Dimensions;

    if constexpr(dim == 1)
        EXPECT_EQ(this->m_Grid.maxIndex(), (3) - 1);
    else if constexpr(dim == 2)
        EXPECT_EQ(this->m_Grid.maxIndex(), (3 * 5) - 1);
    else if constexpr(dim == 3)
        EXPECT_EQ(this->m_Grid.maxIndex(), (3 * 5 * 7) - 1);
    else if constexpr(dim == 4)
        EXPECT_EQ(this->m_Grid.maxIndex(), (3 * 5 * 7 * 11) - 1);
}
...
TYPED_TEST(GridFixture, IndicesFromPosition)
{
    ...
    EXPECT_EQ(*std::crbegin(indices), this->m_Grid.maxIndex());
}
